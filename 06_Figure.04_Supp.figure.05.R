...
### Panel B: Beta diversity, bray-curtis
...
dir = ".../IL17.TNF/ITS/jobs/1_alpha.div_boxplots_R/"

# list of phyloseq objects to process
pseqs <- list(physeq = phy_ITS.R1_human_TNF.B.C_IL17.B.C.D_fR1_even1000)

# list of phyloseq names
pseq.names <- c("ITS.R1_human_TNF.B.C_IL17.B.C.D_fR1_even1000")
...
for (i in seq_along(pseqs)) {
  ...
  for (j in seq_along(divs)) {
    ...
    filename_table.for.plot = paste(pseq.names[i], "adiv", divs[j], "table_for.plott.csv", sep = "_")
    ...
    if(divs[j] == "Observed") {
      p <- p + ylab("Observed OTUs") + expand_limits(y = c(0, max(d$Diversity)+10))
    } else if(divs[j] == "Shannon") {
      p <- p + ylab("Shannon index") + expand_limits(y = c(0, max(d$Diversity)+0.5))
    } else if(divs[j] == "Simpson") {
      p <- p + ylab("Simpson index") + expand_limits(y = c(0, max(d$Diversity)+0.1))
    }
...
# directory for storing files
dir = ".../IL17.TNF/ITS/jobs/2_beta.div_pcoa_R/"
...
# directory for storing files
dir = ".../IL17.TNF/ITS/jobs/2_beta.div_pre.post.tx_R/"
...
# colors
col <- c("#ee8fac", "#cd718c", "#ffb8c2", "#ff959f", "#ff7981", "#e3656a", "#f3585b",
         "#d96e5c", "#ff7359", "#be7d6d", "#ffad90", "#c47b61", "#b48a7a", "#ff7b48",
         "#de6d38", "#f08323", "#c17e4b", "#ffa967", "#ffba7b", "#ffcc9f", "#ca7c00",
         "#ffb657", "#b58517", "#f8e0bd", "#ffde7f", "#eeca33", "#ffebaa", "#ffef63",
         "#f2f45a", "#b0c62a", "#e6ff7d", "#7fa700", "#759929", "#ebffb8", "#6fa70b",
         "#79974a", "#8ac536", "#caff7e", "#efffd9", "#81936f", "#bdff8c", "#91f06f",
         "#4ca039", "#4bbc45", "#69db63", "#3da04e", "#b8ffbc", "#8dffa0", "#6efb8d",
         "#00be59", "#22e181", "#4f9d6f", "#679879", "#37fca1", "#00b57b", "#00f8b1",
         "#65ffc3", "#01cd93", "#00a77c", "#a5ffde", "#02f4c7", "#87ffde", "#00a488",
         "#02d6b5", "#01f0d5", "#02c5b4", "#2c9d92", "#02e9f1", "#72f8ff", "#69a5ac",
         "#a6f3ff", "#3a99ae", "#4ed9ff", "#009cbf", "#01aee6", "#47b4ff", "#6791ba",
         "#98beff", "#95abff", "#cbcfff", "#8588ff", "#947fdd", "#b195ff", "#9783c1",
         "#e2c7ff", "#d5a7ff", "#ab76da", "#e4b8ff", "#b476c8", "#dd7efb", "#aa90b0",
         "#b17bbb", "#e395f2")
...
# directory for storing files
dir = ".../IL17.TNF/ITS/jobs/3_taxa.summary_R/"
...
for (i in seq_along(pseqs)) {
  ...
  p <- plot_bar(tm, x = "Sample", fill = "Order") + 
    scale_fill_manual(breaks = top.taxa, values = c) +
    scale_x_discrete(labels = c("TNFi\npre", "TNFi\nmaint", "IL-17i\npre", "IL-17i\nload", "IL-17i\nmaint")) +
